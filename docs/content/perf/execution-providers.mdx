...
In order to configure sessions to use execution providers, you must **register** them when creating an environment or session. You can do this via the [`SessionBuilder::with_execution_providers`](https://docs.rs/ort/latest/ort/session/builder/struct.SessionBuilder.html#method.with_execution_providers) or [`EnvironmentBuilder::with_execution_providers`](https://docs.rs/ort/latest/ort/environment/struct.EnvironmentBuilder.html#method.with_execution_providers) methods. For example, to register the CUDA execution provider for a session:
...
fn main() -> anyhow::Result<()> {
	let env = ort::init().build()?;
    let builder = Session::builder(&env)?;
...
You can also check whether a custom-linked build of ONNX Runtime was even compiled with support for the execution provider with the [`is_available`](https://docs.rs/ort/latest/ort/ep/trait.ExecutionProvider.html#method.is_available) method.
...
## Global EPs
You can configure EPs to be registered for all sessions created throughout the program by configuring them on the [environment](/fundamentals/environment):
...
fn main() -> anyhow::Result<()> {
    ...
    let session = Session::builder(&env)?.commit_from_file("model.onnx")?;
    // The session will attempt to register the CUDA EP since we configured environment EPs.
