...
impl Kernel for CustomOpOneKernel {
	fn compute(&mut self, ctx: &KernelContext) -> Result<()> {
		let x = ctx.input(0)?.ok_or_else(|| crate::Error::wrap(MissingInput))?;
		let y = ctx.input(1)?.ok_or_else(|| crate::Error::wrap(MissingInput))?;
		let (x_shape, x) = x.try_extract_raw_tensor::<f32>()?;
		let (y_shape, y) = y.try_extract_raw_tensor::<f32>()?;

		let mut z = ctx.output(0, x_shape)?.ok_or_else(|| crate::Error::wrap(MissingInput))?;
...
impl Kernel for CustomOpTwoKernel {
	fn compute(&mut self, ctx: &KernelContext) -> crate::Result<()> {
		let x = ctx.input(0)?.ok_or_else(|| crate::Error::wrap(MissingInput))?;
		let (x_shape, x) = x.try_extract_raw_tensor::<f32>()?;
		let mut z = ctx.output(0, x_shape.clone())?.ok_or_else(|| crate::Error::wrap(MissingInput))?;
